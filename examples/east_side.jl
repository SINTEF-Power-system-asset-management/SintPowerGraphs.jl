...
mpc.branch = coalesce.(join(mpc.branch, mpc.reldata, on = [:f_bus, :t_bus], kind=:left), 0.0)
...
for b_type in [:switch, :indicator, :transformer]
	for branch in eachrow(getfield(mpc, b_type))
		deleterows!(mpc.branch,
					findall((mpc.branch.f_bus .== branch.f_bus) .&
							(mpc.branch.t_bus .== branch.t_bus)))
	end
end

# Rename stuff to be consistent with FaSad
rename_branch = Dict(:f_bus => :from, :t_bus => :to,
					 :rateA => :apparent_power_limit)
...
# Add component names
mpc.branch.name = [string("L", x) for x in 1:nrow(mpc.branch)]
...
branch_h = [:from, :to, :apparent_power_limit, :name, :repair_time,
			:failure_frequency_temporary, :failure_frequency_permanent,
			:length]
