...
function f_process_lines(mpc, mpc_temp)
    branches_columns =
        ["f_bus", "t_bus", "r", "x", "b", "rateA", "rateB", "rateC", "ratio", "angle"]
    reldata_columns = [
        "ID",
        "f_bus",
        "t_bus",
    ...
    for row in collect(eachrow(mpc_temp.lines))
        ...
        reldata_entry = [
            ...
            row["to"],
            row["repair_time"] / 60,
            row["failure_frequency_temporary"],
            row["failure_frequency_permanent"],
            0,
            0.02,
            row["apparent_power_limit"],
        ]
...
function f_process_switch(mpc, mpc_temp)
    ...
    for row in collect(eachrow(mpc_temp.switchgear))
        switches_entry = [
            row["from"],
            row["to"],
            if row["switchgear_type"] == "breaker"
                "True"
            else
                "False"
            end,
...
function f_process_transformers(mpc, mpc_temp)
    ...
    for row in collect(eachrow(mpc_temp.transformers))
        if row["transformer_type"] == "secondary"
            trafos_entry =
                [row["name"], row["from"], row["to"], row["apparent_power_limit"]]
...
function f_process_nodes(mpc, mpc_temp, slack_bus)
    ...
    for row in collect(eachrow(mpc_temp.nodes))
        ...
        if name in mpc_temp.delivery_points[!, "name"]
            df = mpc_temp.delivery_points[
                mpc_temp.delivery_points[!, "name"].==name,
                ["demand", "reference_demand"],
            ]
